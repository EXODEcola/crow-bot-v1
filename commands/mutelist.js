        ...
        if(process.env.owner ===message.author.id || db.get(`ownermd.${message.author.id}`) === true || db.get(`${message.guild.id}.${message.author.id}.wlmd`) === true) {
            ...
            let tdata = await message.channel.send(new Discord.MessageEmbed().setColor(color).setTitle(`Mute en cours`))
                ...
                .setDescription(muteRole.members.map((m,i) => `${m.user}`).slice(p0, p1)   )
                                .setFooter(`${page}/${Math.ceil(muteRole.members.size / 30 || 1)} • discord.gg/exodedev-fivem`)  
                .setColor(color)
                        ...
                        .setDescription(muteRole.members
                           ...
                           .map((m,i) => `${m.user}`)
                            .slice(p0, p1)
                                        ...
                                        .setFooter(`${page}/${Math.ceil(muteRole.members.size / 30)} • discord.gg/exodedev-fivem`)  
